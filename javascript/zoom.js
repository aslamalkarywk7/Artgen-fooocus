...
// إذا تغيرت سمة نمط اللوحة القماشية، فمن خلال الملاحظة يحدث ذلك فقط عندما تتغير الصورة
...
// تطبيق التوسيع التلقائي إذا كان ممكّنًا
          if (hotkeysConfig.canvas_auto_expand) {
            targetElement.addEventListener("mousemove", autoExpand);
            // قم بإعداد مراقب لتتبع تغييرات السمات
            observer.observe(targetElement, { attributes: true, childList: true, subtree: true });
          }

        // التعامل مع الأحداث داخل العنصر targetElement فقط
     ...
     // التعامل مع تعديل حجم الفرشاة مع الضغط على مفتاح ctrl
            if (isModifierKey(e, hotkeysConfig.canvas_hotkey_adjust)) {
                e.preventDefault();

                // زيادة أو تقليل حجم الفرشاة بناءً على اتجاه التمرير
                adjustBrushSize(elemId, e.deltaY);
            }
        });

        // التعامل مع حدث النقل لوظيفة التحريك. يقوم بتحديث متغيرات panX وpanY ويطبق التحويل الجديد على العنصر المستهدف.
        function handleMoveKeyDown(e) {

            // قم بتعطيل أقفال المفاتيح حتى يعمل اللصق من المخزن المؤقت بشكل صحيح
            if ((e.ctrlKey && e.code === 'KeyV') || (e.ctrlKey && e.code === 'KeyC') || e.code === "F5") {
                return;
            }

// قبل تنشيط الاختصار، تأكد من أن المستخدم لا يكتب بشكل نشط في حقل الإدخال
            if (!hotkeysConfig.canvas_blur_prompt) {
...
// كشف مستوى التكبير/التصغير وتحديث سرعة التحريك.
