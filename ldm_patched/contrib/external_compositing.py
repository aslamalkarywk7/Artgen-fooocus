...
#  الكود تقنيات مختلفة لتركيب الصور باستخدام أوضاع تركيب Porter-Duff في PyTorch. فيما يلي تفصيل للمكونات الرئيسية:

# 1. PorterDuffMode Enum:
# يحدد هذا أوضاع التركيب المختلفة لـ Porter-Duff مثل ADD، CLEAR، DARKEN، MULTIPLY، SRC_OVER، وما إلى ذلك. تتحكم هذه الأوضاع في كيفية دمج صورتين (المصدر والوجهة)، خاصةً بناءً على قيم ألفا (الشفافية) الخاصة بهما.
# 2. دالة التركيب ( porter_duff_composite):
# porter_duff_compositeتتضمن الوظيفة :
# src_image:الصورة المصدرية.
# src_alpha:قناع ألفا للصورة المصدر.
# dst_image:صورة الوجهة.
# dst_alpha:قناع ألفا للصورة الوجهة.
# mode:وضع التركيب (من PorterDuffMode).
# يقوم بحساب الصورة المركبة استنادًا إلى الوضع المحدد، مما يؤدي إلى إرجاع صورة مركبة جديدة وقناع ألفا.
# 3. فئات معالجة الصور:
# بورتر دافصورة مركبة:
# تطبق هذه الفئة وضع Porter-Duff المحدد على دفعات من الصور وأقنعة ألفا. كما تتولى أيضًا تغيير حجم الصور المصدر والوجهة وأقنعة ألفا لضمان أن تكون لها نفس الأبعاد.
# تقسيم الصورة مع ألفا:
# تقوم هذه الفئة بتقسيم الصورة إلى مكوناتها RGB وalpha. إذا كانت الصورة تحتوي على أكثر من ثلاث قنوات، فإنها تفترض أن القناة الرابعة هي قناع alpha. إذا لم يتم توفير alpha، فإنها تستخدم قناع all-ones.
# انضم إلى الصورة مع ألفا:
# تقوم هذه الفئة بإعادة دمج صورة RGB مع قناع ألفا في صورة واحدة تحتوي على أربع قنوات (RGB + Alpha).
# 4. وظيفة تغيير حجم القناع ( resize_mask):
# تعمل وظيفة المساعدة هذه على تغيير حجم القناع إلى شكل محدد باستخدام الاستيفاء الخطي الثنائي، وهو أمر مفيد عندما يحتاج قناع ألفا إلى مطابقة حجم الصورة المقابلة.
# 5. NODE_CLASS_MAPPINGS وNODE_DISPLAY_NAME_MAPPINGS:
# تُستخدم هذه التعيينات لتسجيل فئات العقد وأسماء العرض الخاصة بها في الإطار الأوسع. وهذا مفيد عند دمج هذه الوظائف في أنظمة تعتمد على واجهة المستخدم مثل ComfyUI.
# يعد هذا الكود مثاليًا للتطبيقات التي تتطلب تكوين صور دقيقة، مثل أدوات تحرير الصور، حيث يكون وضع طبقات من الصور بمستويات شفافية مختلفة أمرًا ضروريًا.
