...
# يبدو أن كود Python المقدم يتضمن تحميل وترميز بيانات الصورة باستخدام PhotoMakerIDEncoder ، والذي يبدو أنه يعتمد على نماذج رؤية CLIP مع تعديلات محددة. فيما يلي نظرة عامة على المكونات الرئيسية:

# MLP وFuseModule :

# MLP:ينفذ إدراكًا متعدد الطبقات، مع اتصالات متبقية اختيارية. ويتكون من طبقتين خطيتين مع تنشيط GELU.
# FuseModule:يدمج تضمينات المطالبة مع تضمينات المعرف (التي من المحتمل أن تمثل ميزات مرئية)، ويعالجها من خلال MLPs اثنين.
# برنامج PhotoMakerIDencoder :

# مشفر مخصص يعالج بيانات الصورة ويدمجها مع تضمينات المطالبة. يعتمد هذا النموذج على بنية محول الرؤية المحددة بواسطة VISION_CONFIG_DICTويمتد إلى نماذج رؤية CLIP. ويستخدم fuse_moduleلدمج التضمينات المرئية مع تضمينات النص المطالبة.
# برنامج PhotoMakerLoader :

# يتولى تحميل PhotoMakerIDEncoderالنموذج من مسار محدد. يتم تهيئة النموذج وتحميل حالته من ملف، على افتراض أن التنسيق يحتوي على مفتاح "id_encoder".
# PhotoMakerEncode :

# يقوم بترميز صورة إلى جانب مطالبة نصية باستخدام نموذج PhotoMaker المحمّل. يقوم أولاً بمعالجة الصورة مسبقًا، وتقسيم النص إلى رموز، وضبط الرموز بناءً على معايير خاصة (مثل رمز معين مثل "photomaker"). بناءً على ما إذا كان الرمز موجودًا في النص، فإنه يضبط تضمينات المطالبة بشكل مشروط باستخدام المشفر ويعيد معلومات التكييف.
# يبدو أن هذا الإعداد مناسب لمهام مثل تحويل النص إلى صورة أو معالجتها، وخاصة دمج المعلومات المرئية في التكييف الفوري عبر التضمينات المكتسبة.
import torch
